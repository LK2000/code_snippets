...
fifo_init(FIFO* fp, int num_entries)
{
    ...
    fp->rd_ptr = 0;
    fp->wr_ptr = 0;
    fp->user_data = (long *) malloc(sizeof(long) * num_entries);

    if (fp->user_data)
    {
        log_debug_low("FIFO created\n");
...
fifo_insert(FIFO* fp, void* data)
{
    ...
    if (next_val == fp->rd_ptr)
    {
        ...
        lp = (long *) malloc(sizeof(long) * (fp->entries + 10));
        if (!lp)
        {
    ...
    log_debug_low("inserting data at index %d\n", fp->wr_ptr);
...
fifo_remove(FIFO* fp)
{
    ...
    log_debug_low("removing data at index %d\n", fp->rd_ptr);
